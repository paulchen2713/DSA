%
% inputstring_2_L_array_hex function
%
function [out, k] = input_string_2_L_array_hex(s_input, d, subtype)
% fixed coefficients
b = 1600;
c = 2 * d;
r = b - c; % rate,  for every r-bit a section
%
s_len = length(s_input) / 2; % in bytes, represent by 2 hex-digit
s = s_input;
% no need to convert to hex, like normal text input
% for is = 1 : s_len
