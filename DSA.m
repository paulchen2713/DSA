...
clc;
%
% determine an N-bit prime number q
% determine a  L-bit prime number p, such that p-1 is a multiple of q, q | p-1
%
L = 26;
N = 12;
[p, q] = prime_pairing_DSA(L, N);
%
% determine an integer h randomly, 1 < h < p-1
%
...
%
% generate a private key x randomly, 0 < x < q
%
...
%
m = 'document';
HASH_type = 'SHA3-512'; % the HASH type
HASH_len = 512;         % the HASH length
m_SHA = SHA3_text(m, HASH_type, HASH_len);
m_SHA = hex2dec(m_SHA(1:8)); % only take first N-bit of the hash value
%
...
%
% compute w, such that w * s = 1 mod q, w as s inverse
%
uw = s;
vw = q;
x1w = 1;
...
%
%
fprintf('The public keys are:  [p, q, g, y] = [%d, %d, %d, %d]\n', p, q, g, y);
fprintf('The private keys are: [x, k] = [%d, %d]\n', x, k);
%
%
